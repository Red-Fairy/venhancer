...
@torch.no_grad()
def sample_dpmpp_2m_sde(noise, model, sigmas, eta=1.0, s_noise=1.0, solver_type="midpoint", show_progress=True, rotation_decoding=False):
    """
    DPM-Solver++ (2M) SDE.
    """
    ...
    for i in trange(len(sigmas) - 1, disable=not show_progress):
        ...
        # roll the x by 1/4 width
        if rotation_decoding:
            x = x.roll(x.shape[-1] // 4, dims=-1)
            denoised = denoised.roll(denoised.shape[-1] // 4, dims=-1)
            old_denoised = old_denoised.roll(old_denoised.shape[-1] // 4, dims=-1)
            rolled_total += 1

    # roll back the x by rolled_total * 1/4 width   
    if rotation_decoding:
        rolled_total = rolled_total % 4
        x = x.roll(-rolled_total * x.shape[-1] // 4, dims=-1)
    return x

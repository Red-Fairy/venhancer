...
class VideoToVideo:
    ...
    def test(
        ...
        with amp.autocast(enabled=True):
            ...
            # debug, save the latent (gen_vid)
            # torch.save(gen_vid, "gen_vid.pt")

        w1, w2, h1, h2 = padding
        gen_video = gen_video[:, :, :, h1 : h + h1, w1 : w + w1]

        if blended_decoding: # roll the gen_vid by half and blend the two decodede videos
            gen_video_rolled = self.tiled_chunked_decode(gen_vid.roll(gen_vid.shape[-1] // 2, dims=-1)).roll(-video_data.shape[-1] // 2, dims=-1)
            gen_video_rolled = gen_video_rolled[:, :, :, h1 : h + h1, w1 : w + w1]

            blend_pixels = w // 4  - w // 32
            weight_left_half = torch.cat([torch.zeros(blend_pixels), torch.linspace(0, 1, w // 2 - 2 * blend_pixels), torch.ones(blend_pixels)]).to(gen_video.device)
            weight_left = torch.cat([weight_left_half, weight_left_half.flip(0)]).view(1, 1, 1, 1, -1) # (1, 1, 1, 1, W)
            weight_right = 1 - weight_left

            gen_video = gen_video * weight_left + gen_video_rolled * weight_right

        # set the first and last rows to be the same pixel value (mean of the second and second last rows)
        gen_video[:, :, :, 0, :] = gen_video[:, :, :, 1, :].mean(dim=-1, keepdim=True)
        gen_video[:, :, :, -1, :] = gen_video[:, :, :, -2, :].mean(dim=-1, keepdim=True)

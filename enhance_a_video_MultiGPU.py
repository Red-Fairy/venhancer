...
class VEnhancer:
    ...
    def enhance_a_video(self, video_path, prompt, up_scale=4, target_fps=24, noise_aug=300, blended_decoding=False):

        print(f"enhance_a_video: {video_path} with prompt: {prompt}")
        ...
        drop_prefix_cnt = 0 if in_f_num % 4 == 0 else 4 - in_f_num % 4
        ...
        if get_context_parallel_rank() == 0:
            if self.result_dir is None:
                self.result_dir = os.path.dirname(video_path)
            save_video(output, self.result_dir, f"{save_name}_enhanced.mp4", fps=target_fps, drop_prefix_cnt=drop_prefix_cnt)
        dist.barrier()
...
def main():
    ...
    for ind, file_path in enumerate(file_path_list):
        ...
        venhancer.enhance_a_video(file_path, prompt, up_scale, target_fps, noise_aug, blended_decoding)
